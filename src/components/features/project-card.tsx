...
export const ProjectCard = memo(function ProjectCard(project: ProjectCardProps) {
  ...
  return (
    <motion.div {...cardMotion} className="h-full">
      <div className="group h-full parchment-panel dark:glass-panel rounded-3xl border-2 border-leather-caramel/30 dark:border-gold-400/30 overflow-hidden card-lift flex flex-col justify-between relative shadow-xl">
        ...
        <div className="relative h-48 sm:h-52 overflow-hidden bg-parchment-subtle dark:bg-surface-primary border-b border-leather-caramel/20 dark:border-gold-400/20">
          ...
          <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-parchment-base/95 dark:bg-black/85 px-3.5 py-1 rounded-full border border-leather-caramel/40 dark:border-gold-400/30 shadow-md">
            <span className="text-[#DFAE2A] dark:text-gold-400 text-xs tracking-tight drop-shadow-[0_0_4px_rgba(201,154,78,0.8)]">
              {is5Star ? "★★★★★" : "★★★★☆"}
            </span>
            <span className="font-display text-[9px] text-leather-dark dark:text-gold-200 font-bold uppercase">
              ARTIFACT
            </span>
          </div>

          {/* Top-Left: Official Elemental Vision Medallion */}
          <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-parchment-base/90 dark:bg-black/70 backdrop-blur-md px-3 py-1 rounded-full border border-leather-caramel/40 dark:border-gold-400/40 shadow-md">
        ...
        <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
          <div>
            ...
            <p className="mt-2 text-xs sm:text-sm leading-relaxed text-[#2A1A0E] dark:text-text-muted font-body line-clamp-3">
              {project.description}
            </p>
            ...
            <div className="mt-4 flex flex-wrap gap-1.5" aria-label="Technologies used">
              {tags.slice(0, 4).map((tag) => (
                <span
                  key={tag}
                  className="px-2.5 py-0.5 rounded-full bg-leather-caramel/10 dark:bg-gold-400/10 border border-leather-caramel/25 dark:border-gold-400/25 text-leather-dark dark:text-platinum-200 text-[10px] font-mono font-bold"
                >
                  {tag}
                </span>
              ))}
